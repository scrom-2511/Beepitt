...
const Signin = () => {
    const [displayComponent, setDisplayComponent] = useState<number>(1);
    return (
        <div className="h-full w-full ">
            <NavigationBar position="static" />
            {displayComponent === 1 && <SigninComponent setDisplayComponent={setDisplayComponent}/>}
            {displayComponent === 2 && <UsernameUpdateComponent/>}
        </div>
    )
}

const SigninComponent = ({setDisplayComponent}:{setDisplayComponent:React.Dispatch<React.SetStateAction<number>>}) => {
    ...
    const handleOnClickBtn = async () => {
        ...
        if (success && "username" in resData) {
            console.log("username is ",resData.username)
            if (resData.username === "") {
                setDisplayComponent(2)
            }
            else {
                dispatch(setUsername(resData.username))
                navigate("/dashboard")
            }
    ...
    return (
        <div className="h-[600px] w-[500px] bg-third rounded-2xl absolute left-1/2 transform -translate-x-1/2 top-1/2 -translate-y-1/2">
            ...
            <div className="h-full px-20 py-15 text-secondary flex flex-col items-center">
                ...
                <input type="text" placeholder="Enter your email" autoFocus className="font-roboto font-light text-sm h-10 w-full border border-secondary rounded-[10px] mb-8 p-5" onChange={(e: React.ChangeEvent<HTMLInputElement>) => dispatch(setEmail(e.target.value))} />
