...
const SignupComponent = ({setComponent}:{setComponent:React.Dispatch<React.SetStateAction<number>>}) => {
  ...
  return (
    <div className="h-[600px] w-[500px] bg-third rounded-2xl absolute left-1/2 transform -translate-x-1/2 top-1/2 -translate-y-1/2">
      ...
      <div className="h-full px-20 py-15 text-secondary flex flex-col items-center">
        ...
        <input type="text" autoFocus placeholder="Enter your email" className="font-roboto font-light text-sm h-10 w-full border border-secondary rounded-[10px] mb-8 p-5" onChange={(e: React.ChangeEvent<HTMLInputElement>) => dispatch(setEmail(e.target.value))} />
